...
$2.5\times$ on the multimodal target. Two structural limits remain and are now
well characterised: the multimodal valley is an expressivity floor of a single
Gaussian state (neither objective reweighting nor encoding removes it), and the
detector should be matched to the target's concentration. The natural next step
is added expressivity --- displacement (already prototyped) or a small mixture
of GBS states --- evaluated under the honest, sample-only protocol and the
analytic KL used here.
